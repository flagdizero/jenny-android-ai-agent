...
class TestAutoCompact:
    ...
    @pytest.mark.asyncio
    async def test_check_expired_ignores_non_unified_sessions(self, tmp_path):
        """Only the unified session is considered; internal sessions stay untouched."""
        loop = _make_loop(tmp_path, session_ttl_minutes=15)
        s1 = loop.sessions.get_or_create("internal:expired")
        ...
        expired_after = loop.sessions.get_or_create("internal:expired")
        assert len(expired_after.messages) == 1
        await loop.close_background_tasks()
    ...
    async def test_auto_compact_stores_summary(self, tmp_path):
        ...
        entry = loop.auto_compact._summaries.get("internal:test")
...
class TestAutoCompactIdleDetection:
    ...
    async def test_auto_compact_does_not_affect_priority_commands(self, tmp_path):
        ...
        raw = "/stop"
        msg = InboundMessage(channel="internal", sender_id="user", chat_id="test", content=raw)
        ctx = CommandContext(msg=msg, session=session, key="internal:test", raw=raw, loop=loop)
...
class TestAutoCompactSystemMessages:
    ...
    async def test_auto_compact_triggers_for_system_messages(self, tmp_path):
        ...
        msg = InboundMessage(
            channel="system", sender_id="subagent", chat_id="internal:test",
...
class TestAutoCompactIntegration:
    ...
    async def test_full_lifecycle(self, tmp_path):
        ...
        msg = InboundMessage(
            channel="internal", sender_id="user", chat_id="test",
...
class TestProactiveAutoCompact:
    ...
    async def test_no_check_when_ttl_disabled(self, tmp_path):
        ...
        session_after = loop.sessions.get_or_create("unified:default")
    ...
    async def test_proactive_archive_on_idle_tick(self, tmp_path):
        ...
        assert len(session_after.messages) == loop.auto_compact._RECENT_SUFFIX_MESSAGES
        assert len(archived_messages) == 2
        entry = loop.auto_compact._summaries.get("unified:default")
        assert entry is not None
    ...
    async def test_proactive_archive_error_does_not_block(self, tmp_path):
        ...
        assert "unified:default" not in loop.auto_compact._archiving
        await loop.close_background_tasks()
    ...
    async def test_proactive_archive_skips_empty_sessions(self, tmp_path):
        ...
        assert "unified:default" not in loop.auto_compact._summaries
        await loop.close_background_tasks()
    ...
    async def test_skip_expired_session_with_active_agent_task(self, tmp_path):
        ...
        await self._run_check_expired(loop, active_session_keys={"unified:default"})
        assert _fake_compact.state["count"] == 0
    ...
    async def test_session_can_be_compacted_again_after_new_messages(self, tmp_path):
        ...
        # First compact cycle
        await loop.auto_compact._archive("unified:default")
        assert _fake_compact.state["count"] == 1

        # User returns, sends new messages
        msg = InboundMessage(channel="internal", sender_id="user", chat_id="test", content="second topic")
        await loop._process_message(msg)

        # Simulate idle again
        loop.sessions.invalidate("unified:default")
        session2 = loop.sessions.get_or_create("unified:default")
...
class TestSummaryPersistence:
    ...
    async def test_summary_recovered_after_restart(self, tmp_path):
        ...
        reloaded = loop.sessions.get_or_create("internal:test")
        assert len(reloaded.messages) == loop.auto_compact._RECENT_SUFFIX_MESSAGES
        _, summary = loop.auto_compact.prepare_session(reloaded, "internal:test")
    ...
    async def test_metadata_persists_for_restart(self, tmp_path):
        ...
        _, summary = loop.auto_compact.prepare_session(reloaded, "internal:test")
        assert summary is not None
        _, summary2 = loop.auto_compact.prepare_session(reloaded, "internal:test")
    ...
    async def test_metadata_cleanup_on_inmemory_path(self, tmp_path):
        ...
        assert "internal:test" in loop.auto_compact._summaries
        loop.sessions.invalidate("internal:test")
        reloaded = loop.sessions.get_or_create("internal:test")
        assert "_last_summary" in reloaded.metadata
    ...
    async def test_new_summary_overrides_old(self, tmp_path):
        ...
        _, summary1 = loop.auto_compact.prepare_session(
            loop.sessions.get_or_create("internal:test"), "internal:test"
        )
        assert summary1 is not None
        assert "First summary." in summary1
        assert "internal:test" not in loop.auto_compact._summaries  # popped by hot path
    ...
    async def test_new_command_clears_last_summary(self, tmp_path):
        ...
        fresh = loop.sessions.get_or_create("internal:test")
