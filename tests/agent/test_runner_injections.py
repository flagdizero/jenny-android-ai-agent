...
async def test_drain_injections_extracts_content_from_inbound_messages():
    ...
    msgs = [
        InboundMessage(channel="internal", sender_id="u", chat_id="c", content="hello"),
        InboundMessage(channel="internal", sender_id="u", chat_id="c", content="world"),
    ]
...
async def test_drain_injections_passes_limit_to_callback_when_supported():
    ...
    msgs = [
        InboundMessage(channel="internal", sender_id="u", chat_id="c", content=f"msg{i}")
...
async def test_drain_injections_skips_empty_content():
    ...
    msgs = [
        InboundMessage(channel="internal", sender_id="u", chat_id="c", content=""),
        InboundMessage(channel="internal", sender_id="u", chat_id="c", content="   "),
        InboundMessage(channel="internal", sender_id="u", chat_id="c", content="valid"),
    ]
...
async def test_checkpoint1_injects_after_tool_execution():
    ...
    await injection_queue.put(
        InboundMessage(channel="internal", sender_id="u", chat_id="c", content="follow-up question")
    )
...
async def test_checkpoint2_injects_after_final_response_with_resuming_stream():
    ...
    await injection_queue.put(
        InboundMessage(channel="internal", sender_id="u", chat_id="c", content="quick follow-up")
    )
...
async def test_injection_cycles_capped_at_max():
    ...
    async def inject_cb():
        ...
        if drain_count["n"] <= _MAX_INJECTION_CYCLES:
            return [InboundMessage(channel="internal", sender_id="u", chat_id="c", content=f"msg-{drain_count['n']}")]
        return []
...
async def test_pending_queue_cleanup_on_dispatch(tmp_path):
    ...
    assert effective_key == UNIFIED_SESSION_KEY
    # The queue should not exist before dispatch
    assert effective_key not in loop._pending_queues
...
async def test_waiting_dispatch_does_not_replace_active_pending_queue(tmp_path):
    ...
    # Il task "attivo" tiene il lock condiviso della sessione.
    async with lock:
        active_pending = asyncio.Queue(maxsize=1)
        loop._pending_queues[session_key] = active_pending

        # Un secondo dispatch per la stessa key deve bloccarsi sul lock e NON
        # deve sostituire la pending-queue attiva mentre attende.
        waiting = asyncio.create_task(
            loop._dispatch(
                InboundMessage(channel="internal", sender_id="u", chat_id="c", content="queued")
            )
        )
        await asyncio.sleep(0.05)  # lascia che raggiunga e si blocchi sul lock
...
async def test_followup_routed_to_pending_queue(tmp_path):
    ...
    msg = InboundMessage(channel="websocket", sender_id="u", chat_id="c", content="follow-up")
...
async def test_pending_queue_full_falls_back_to_queued_task(tmp_path):
    ...
    async def _dispatch(_msg, _pending=None):
        dispatched.set()

    loop._dispatch = AsyncMock(side_effect=_dispatch)  # type: ignore[method-assign]

    pending = asyncio.Queue(maxsize=1)
    pending.put_nowait(InboundMessage(channel="internal", sender_id="u", chat_id="c", content="already queued"))
    loop._pending_queues["internal:c"] = pending

    run_task = asyncio.create_task(loop.run())
    msg = InboundMessage(channel="internal", sender_id="u", chat_id="c", content="follow-up")
    await loop.bus.publish_inbound(msg)
...
async def test_dispatch_republishes_leftover_queue_messages(tmp_path):
    ...
    loop._pending_queues[session_key] = pending
    pending.put_nowait(InboundMessage(channel="internal", sender_id="u", chat_id="c", content="leftover-1"))
    pending.put_nowait(InboundMessage(channel="internal", sender_id="u", chat_id="c", content="leftover-2"))
...
async def test_drain_injections_on_fatal_tool_error():
    ...
    async def chat_with_retry(*, messages, **kwargs):
        ...
        if call_count["n"] == 1:
            return LLMResponse(
                content="",
                tool_calls=[ToolCallRequest(id="c1", name="python_exec", arguments={"cmd": "bad"})],
                usage={},
            )
    ...
    await injection_queue.put(
        InboundMessage(channel="internal", sender_id="u", chat_id="c", content="follow-up after error")
    )
...
async def test_drain_injections_on_llm_error():
    ...
    await injection_queue.put(
        InboundMessage(channel="internal", sender_id="u", chat_id="c", content="follow-up after LLM error")
    )
...
async def test_drain_injections_on_empty_final_response():
    ...
    await injection_queue.put(
        InboundMessage(channel="internal", sender_id="u", chat_id="c", content="follow-up after empty")
    )
...
async def test_drain_injections_on_max_iterations():
    ...
    await injection_queue.put(
        InboundMessage(channel="internal", sender_id="u", chat_id="c", content="follow-up after max iters")
    )
