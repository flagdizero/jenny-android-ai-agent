...
def _make_mock_loop(**overrides):
    ...
    loop.android_web_config.enable = True
    loop.android_web_config.search = MagicMock()
    # Vedi test_settings_api.py: niente fixture in stile `sk-...`, fanno
    # scattare i secret scanner su un repo pubblico.
    loop.android_web_config.search.api_key = "not-a-real-credential"

    # Tools registry mock
    loop.tools = MagicMock()
    loop.tools.tool_names = ["read_file", "write_file", "python_exec", "web_search", "self"]
...
class TestInspectPathNavigation:
    ...
    async def test_inspect_config_subfield(self):
        ...
        loop.android_web_config.enable = True
        tool = _make_tool(runtime_state=loop)
        result = await tool.execute(action="check", key="android_web_config.enable")
    ...
    async def test_inspect_nested_config_redacts_sensitive_scalar_fields(self):
        ...
        loop = _make_mock_loop()
        loop.android_web_config = MagicMock()
        loop.android_web_config.search = SearchConfig()
        tool = _make_tool(loop)

        result = await tool.execute(action="check", key="android_web_config.search")
...
class TestModifyFree:
    ...
    @pytest.mark.asyncio
    async def test_modify_non_whitelisted_runtime_attr_is_denied(self):
        """Fase 4.1 fail-closed: un attributo REALE del loop non in RESTRICTED
        NON è settabile (prima veniva accettato via setattr)."""
        tool = _make_tool()
        result = await tool.execute(action="set", key="provider_retry_mode", value="persistent")
        assert "Error" in result
        assert "not settable" in result
        assert tool._runtime_state.provider_retry_mode == "standard"

    @pytest.mark.asyncio
    async def test_newly_added_loop_attribute_is_not_settable_by_default(self):
        """Proprietà di sicurezza (fail-closed): un attributo NUOVO aggiunto al
        runtime state NON diventa automaticamente modificabile — serve inserirlo
        nell'allowlist RESTRICTED. (Prima la denylist BLOCKED lasciava passare
        qualunque attributo nuovo.)"""
        tool = _make_tool()
        # Simula un nuovo attributo reale del loop, non presente in RESTRICTED.
        tool._runtime_state.newly_added_field = "orig"
        result = await tool.execute(action="set", key="newly_added_field", value="hacked")
        assert "Error" in result
        assert "not settable" in result
        assert tool._runtime_state.newly_added_field == "orig"
    ...
    @pytest.mark.asyncio
    async def test_modify_existing_attr_denied_before_type_check(self):
        """Un attributo reale non whitelisted è negato a priori (fail-closed),
        prima ancora di qualsiasi controllo di tipo."""
        tool = _make_tool()
...
class TestModifyOpen:
    ...
    @pytest.mark.asyncio
    async def test_modify_workspace_denied_fail_closed(self):
        """Fase 4.1: `workspace` è un attributo reale non whitelisted →
        non settabile (fail-closed)."""
        tool = _make_tool()
...
class TestSensitiveSubFieldBlocking:
    ...
    async def test_inspect_api_key_blocked(self):
        """android_web_config.search.api_key must not be accessible."""
        tool = _make_tool()
        result = await tool.execute(action="check", key="android_web_config.search.api_key")
        assert "not accessible" in result
    ...
    async def test_modify_api_key_blocked(self):
        """android_web_config is READ_ONLY, so any set under it is blocked."""
        tool = _make_tool()
        result = await tool.execute(action="set", key="android_web_config.search.api_key", value="evil")
        # Blocked either by READ_ONLY (android_web_config) or sensitive name (api_key)
    ...
    async def test_inspect_non_sensitive_subfield_allowed(self):
        """android_web_config.enable should still be inspectable."""
        tool = _make_tool()
        result = await tool.execute(action="check", key="android_web_config.enable")
        assert "True" in result
...
class TestSecurityAttributeProtection:
    ...
    @pytest.mark.asyncio
    async def test_modify_android_web_config_blocked(self):
        """android_web_config is READ_ONLY — cannot be modified."""
        tool = _make_tool()
        result = await tool.execute(action="set", key="android_web_config", value=MagicMock())
        assert "read-only" in result
    ...
    @pytest.mark.asyncio
    async def test_inspect_android_web_config_allowed(self):
        """android_web_config is READ_ONLY — check should work."""
        tool = _make_tool()
        result = await tool.execute(action="check", key="android_web_config")
        assert "Error" not in result
    ...
    @pytest.mark.asyncio
    async def test_modify_android_web_config_dotpath_blocked(self):
        """android_web_config.enable = False should be blocked because android_web_config is READ_ONLY."""
        tool = _make_tool()
        result = await tool.execute(action="set", key="android_web_config.enable", value=False)
        assert "read-only" in result
