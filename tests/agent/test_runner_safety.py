...
def test_is_ssrf_violation_recognizes_private_url_blocks():
    ...
    from jenny.agent.tool_error_policy import is_ssrf_violation
    ...
    provider.chat_with_retry = AsyncMock(side_effect=[
        LLMResponse(
            ...
            tool_calls=[ToolCallRequest(
                id="call_ssrf",
                name="python_exec",
                arguments={"code": "import os; os.system('curl http://169.254.169.254')"},
            )],
        ),
        LLMResponse(
        ...
        if provider.chat_with_retry.await_count == 1:
            return LLMResponse(
                ...
                tool_calls=[ToolCallRequest(
                    id="call_blocked",
                    name="python_exec",
                    arguments={"code": "import os; os.system('rm scratch.txt 2>/dev/null')"},
                )],
            )
...
async def test_runner_throttles_repeated_workspace_bypass_attempts():
    ...
    bypass_attempts = [
        ToolCallRequest(
            id=f"a{i}", name="python_exec",
            arguments={"code": f"import os; os.system('cat /Users/x/Downloads/01.md  # try {i}')"},
        )
        for i in range(4)
    ]
