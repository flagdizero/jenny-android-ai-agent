...
function discendenti(el) {
  const all = [];
  for (const c of el.children) all.push(c, ...discendenti(c));
  return all;
}
function crea(tag, { id, cls, text } = {}) {
  const el = {
    ...
    cloneNode(profondo) {
      const c = crea(tag, { text: this.text });
      c.attrs = { ...this.attrs };
...
const thread = crea('div', { id: 'home-thread', cls: 'home-thread' });
...
const field = crea('textarea', { id: 'home-input', cls: 'home-input' });
const label = crea('label', { cls: 'home-field' });
label.setAttribute('for', 'home-input');
composer.appendChild(label);
composer.appendChild(field);
for (const x of [thread, vuoto, lavoro, rete, allegati, composer]) chat.appendChild(x);
p0.appendChild(chat);
...
function write(...names) {
  ...
  for (const n of names) thread.appendChild(crea('div', { cls: 'home-msg', text: n }));
}
const messaggiDi = (el) => el.querySelectorAll('.home-msg').map((m) => m.text);
...
function change(key) {
  ...
  letture.push({ key, finisci, resolve });
  return p;
}
let atBottom = 0;
write(...storie.A);
...
def test_the_page_it_leaves_keeps_a_photo_of_what_it_was() -> None:
    ...
    _run(
        "t.arrives(p1, 'B');\n"
        "const snapshot = fotoDi(p0);\n"
        "assert.ok(snapshot, 'la pagina 0 e rimasta senza foto');\n"
        "assert.deepEqual(messaggiDi(snapshot), ['A1', 'A2']);\n"
    )
...
def test_a_half_read_chat_is_not_photographed() -> None:
    ...
    **E una lettura vecchia che finisce dopo una nuova non garantisce per
    ...
    """
    _run(
        "let a = t.arrives(p1, 'B'); letture[0].finisci(); await a;\n"
        "a = t.arrives(p0, 'A'); letture[1].finisci(); await a;\n"
...
def test_a_photo_already_there_is_not_printed_again() -> None:
    ...
    _run(
        "t.snapshotIfNeeded(p2, 'C');\n"
        "const before = fotoDi(p2);\n"
        "t.snapshotIfNeeded(p2, 'C');\n"
        "assert.equal(fotoDi(p2), before);\n"
    )
...
def test_the_out_of_place_mark_lasts_one_arrival_only() -> None:
    ...
    _run(
        ...
        "a = t.arrives(p0, 'A'); letture[1].finisci(); await a;\n"
        "a = t.arrives(p2, 'B'); letture[2].finisci(); await a;\n"
