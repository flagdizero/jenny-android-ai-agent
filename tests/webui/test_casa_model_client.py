...
/* Un payload della forma di `/api/settings`, con dentro solo cio' che questa
   stanza legge. */
function settings(providers, active, modello, extra) {
  return {
    providers,
...
async function room(data, catalog) {
  ...
  stanzaModelli.setSettings(data);
  stanzaModelli.open();
...
def test_picking_a_model_saves_the_provider_with_it() -> None:
    ...
    _run_js("""
      ...
      s.pickProvider('anthropic');
      await new Promise((r) => setImmediate(r));
      ultimoPayload = settings(data.providers, 'anthropic', 'claude-x');
...
def test_a_new_key_makes_the_catalogue_be_asked_again() -> None:
    ...
    _run_js("""
      ...
      assert.deepEqual(salvataggi, [{ type: 'provider', name: 'groq', api_key: 'gsk_una_chiave_vera' }]);
...
def test_a_change_that_needs_a_restart_says_so() -> None:
    ...
    _run_js("""
      ...
      ultimoPayload = settings(data.providers, 'groq', 'altro', { requires_restart: true });
      await s.pickModel('altro');
