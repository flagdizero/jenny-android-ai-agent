"""Tests for WebSocketDispatcher delta coalescing to reduce streaming latency."""
import asyncio
...
class MockChannel(WebSocketChannel):
    ...
    def __init__(self, config, bus):
        ...
        self._send_delta_mock = AsyncMock(return_value=[])
    ...
    async def send_delta(self, chat_id, delta, metadata=None, *, only_conns=None, skip_persist=False):
...
def manager(config, bus):
    ...
    manager = WebSocketDispatcher(config, bus)
    ...
    async def test_single_delta_not_coalesced(self, manager, bus):
        ...
        async def process_one():
            try:
                ...
                channel = manager._route_channel(m)
                if channel:
        ...
        manager.channels["websocket"]._send_delta_mock.assert_called_once_with(
...
class TestProgressFiltering:
    ...
    def test_progress_visibility_uses_global_defaults(self, manager):
        assert manager._channel_allows_progress(manager.channels.get("websocket"), tool_hint=False) is True
        assert manager._channel_allows_progress(manager.channels.get("websocket"), tool_hint=True) is False

    def test_progress_visibility_uses_channel_overrides(self, manager):
        manager.channels["websocket"].send_progress = False
        manager.channels["websocket"].send_tool_hints = True

        assert manager._channel_allows_progress(manager.channels.get("websocket"), tool_hint=False) is False
        assert manager._channel_allows_progress(manager.channels.get("websocket"), tool_hint=True) is True

    def test_progress_visibility_returns_false_for_missing_channel(self, config, bus):
        dispatcher = WebSocketDispatcher(config, bus)
        assert dispatcher.channels.get("websocket") is None
        assert dispatcher._channel_allows_progress(dispatcher.channels.get("websocket"), tool_hint=False) is False
        assert dispatcher._channel_allows_progress(dispatcher.channels.get("websocket"), tool_hint=True) is False
