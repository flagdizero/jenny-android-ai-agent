...
> **Raw ≠ summary.** The condensed version of a source lives *here*, in `wiki/summaries/`. The file in `raw/` must stay the source's **verbatim** text — the exact `text` the fetch/read tool returned, copied whole (wrapper and all), never a hand-written recap. Writing a tidy paraphrase into `raw/` (e.g. shrinking a 40 000-char fetch to a clean 3 000-char retelling) poisons the audit trail: every claim then traces back only to your memory, not to a real source. Save the whole raw text; put the short version here.
...
Obsidian and most Markdown viewers render KaTeX natively.
...
6. **Intra-wiki only** — every link must resolve inside this wiki. Never link into another wiki (no `[[../other/...]]`, no bare link that only exists elsewhere). To reference another wiki, name it in prose and point at `wikis/_index.md`; `lint` treats cross-wiki links as dead.
