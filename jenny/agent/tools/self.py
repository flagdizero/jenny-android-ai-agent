...
class MyTool(Tool, ContextAware):
    ...
    def description(self) -> str:
        base = (
            ...
            "(e.g. '_last_usage.prompt_tokens', 'android_web_config.enable').\n"
            "- set (key, value): change config or store notes in your scratchpad. "
            "Scratchpad keys persist across turns but not restarts.\n"
            "Key values: _current_iteration (current progress), "
            "max_iterations - _current_iteration = remaining iterations.\n"
            "Note: android_web_config and exec_config are readable but read-only.\n"
            "\n"
            "When to use:\n"
    ...
    def _modify_free(self, key: str, value: Any) -> str:
        # Capability model fail-closed: un attributo REALE del loop è
        # modificabile SOLO se nell'allowlist RESTRICTED (gestita da _modify
        # prima di arrivare qui) o è ``model_preset``. Se arriviamo qui con un
        # attributo reale, NON è whitelisted → deny. Così un nuovo attributo
        # aggiunto ad AgentLoop è privato per costruzione, non esposto per
        # default (niente più drift della denylist BLOCKED).
        if key != "model_preset" and _has_real_attr(self._runtime_state, key):
            self._audit("modify", f"BLOCKED non-whitelisted runtime attr {key}")
            return (
                f"Error: '{key}' is a runtime attribute and is not settable. Only "
                "whitelisted runtime controls can be modified; free-form keys are "
                "stored in the scratchpad instead."
            )
        # model_preset è gestito dalla sua whitelist dedicata via _runtime_vars.
        if key == "model_preset" and _has_real_attr(self._runtime_state, key):
            old = getattr(self._runtime_state, key)
            try:
                setattr(self._runtime_state, key, value)
